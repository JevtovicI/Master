...
import jwd.apoteka.support.UserDTOToUser;
import jwd.apoteka.support.UserToUserDTO;
import jwd.apoteka.web.dto.UserDTO;
import jwd.apoteka.web.dto.UserRegistrationDTO;
...
//		}
		...
		if (name != null) {
			users = userService.pretragaPoImenuIPrezimenu(name);
		} else {
			Page<User> usersPage = userService.findAll(page);
			if (page > usersPage.getTotalPages()) {
				return new ResponseEntity<List<UserDTO>>(HttpStatus.NOT_FOUND);
